            ...
            if(response.status == 'success') {
                ...
                if(response.jobApply.length > 0) {
                    $(response.jobApply).each(function(index,value) {
                        ...
                        table.row.add(
                            [
                                '<button class="border-0 btn '+active+'" onclick="getRelatedApplicantInfo('+response.seeker.id+','+value.job_post_id+')">'+value.seeker_first_name+' '+value.seeker_last_name+'</button>',
                                '<button class="border-0 btn '+active+'" onclick="getRelatedApplicantInfo('+response.seeker.id+','+value.job_post_id+')">'+moment(value.seeker_applied_date).format("DD/MM/YYYY")+'</button>'
                            ]
                        ).draw();
                        // $(".applicant-receive-table").append('<tr class="applicant-receive-table-tr" onclick="getRelatedApplicantInfo('+response.seeker.id+','+value.job_post_id+')"><td class="'+active+'">'+value.seeker_first_name+' '+value.seeker_last_name+'</td><td class="text-end">'+moment(value.seeker_applied_date).format("DD/MM/YYYY")+'</td></tr>');
