...
class PointRecordController extends Controller
{
    public function usedPointHistory(Request $request)
    {
        ...
        }])->whereIn('employer_id', $employer_id)->whereStatus('Complete')->orderBy('created_at','desc')->select('id','job_post_id','job_apply_id','package_item_id','point','created_at as date')->paginate(15);
    ...
    public function getAllUsedPointHistory(Request $request)
    {
        $employer = Employer::findOrFail($request->user()->id);
    ...
    public function usedPointSearch(Request $request)
    {
        $employer = Employer::findOrFail($request->user()->id);
